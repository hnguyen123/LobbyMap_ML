#===================
#LOBBY MAP: regression analysis at company and sector level
#===================
# REVISED: Hoa Nguyen
...
df_2 <- read.csv("company_sector_region.csv", header = TRUE, stringsAsFactors = FALSE)
...
# Function to run regression and plot results at company level
run_analysis_company <- function(subset_data, query) {
  ...
  p1 <- ggplot(tidy_model_filtered, aes(x = estimate, y = reorder(term, estimate))) +
    ...
    theme_minimal() +
    labs(title = paste("Regional and Yearly Effects (controlled for company) on Evidence Score for", query),
         x = "Estimate",
         y = "Term")
  print(p1)
...
# Function to run regression and plot results at sector level
run_analysis_sector <- function(subset_data, query) {
  # Convert relevant columns to factors
  subset_data$new_region <- factor(subset_data$new_region, levels = c("global", unique(subset_data$new_region)[unique(subset_data$new_region) != "global"]))
  subset_data$sector <- factor(subset_data$sector)
  subset_data$evidence_year <- factor(subset_data$evidence_year, levels = c("2014", unique(subset_data$evidence_year)[unique(subset_data$evidence_year) != "2014"]))
  ...
  p1 <- ggplot(tidy_model_filtered, aes(x = estimate, y = reorder(term, estimate))) +
    ...
    geom_vline(xintercept = 0, linetype = "dashed", color = "red") +
    theme_minimal() +
    labs(title = paste("Regional and Yearly Effects (controlled for secttor) on Evidence Score for", query),
         x = "Estimate",
         y = "Term")
  print(p1)
  
  # Filter and plot for sector effects with p-value < 0.05
  tidy_model_filtered_sector <- tidy_model %>% filter(grepl("sector", term) & p.value < 0.05)
  p2 <- ggplot(tidy_model_filtered_sector, aes(x = estimate, y = reorder(term, estimate))) +
    geom_point() +
    geom_errorbarh(aes(xmin = conf.low, xmax = conf.high)) +
    geom_vline(xintercept = 0, linetype = "dashed", color = "red") +
    theme_minimal() +
...
# Run analysis for each evidence_query category at company and sector levels
unique_queries <- unique(df$evidence_query)
lapply(unique_queries, function(query) {
  subset_data <- df %>% filter(evidence_query == query)
  run_analysis_company(subset_data, query)
  run_analysis_sector(subset_data, query)
})
